...
public:
    perlin()
    {
        ranvec = new vec3[this->point_count];
        for (int i = 0; i < point_count; ++i)
            ranvec[i] = unit_vector(2 * sampler::sampleUnitCube() - vec3(1,1,1));
    ...
    double turb(vec3 const &p, int depth = 7)
    {
        auto accum = 0.;
        auto temp_p = p;
        auto weight = 1.0;
...
private:
    ...
    {
        for  (int i = 0; i < perlin::point_count; ++i)
            std::swap(p[i], p[randomint(i, perlin::point_count - 1)]);
    }
